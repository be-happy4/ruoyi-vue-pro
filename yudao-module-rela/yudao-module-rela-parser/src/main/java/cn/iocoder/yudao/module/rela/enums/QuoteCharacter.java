...
package cn.iocoder.yudao.module.rela.enums;
...
public enum QuoteCharacter {
    ...
    private static final Map<Character, QuoteCharacter> BY_FIRST_CHAR = Arrays.stream(values())
            .filter(x -> x != NONE)
            .collect(Collectors.toUnmodifiableMap(x -> x.startDelimiter.charAt(0), x -> x));
    ...
    public String unwrap(final String value) {
        return isWrapped(value) ? value.substring(startDelimiter.length(), value.length() - endDelimiter.length())
                : value;
    }
